...
1. `while`循环
   ...
   a. 如果表达式expression为非零值（表示真），则执行statement语句；如果                       expression为零（表示伪）就跳出循环，不再执行循环体。
   b. 循环上面步骤
...
2. `switch case`
   ...
   - case 的 value 必须与 switch 中的变量具有相同的数据类型，且必须是一个常量或字面量。
